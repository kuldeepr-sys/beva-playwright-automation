...
class LoginPage extends BasePage {
  ...
  constructor(page) {
    super(page);
    // ── Fields ─────────────────────────────────────────────────────────
    // Real placeholder on live site is "Email Address" (confirmed from diagnostic)
    this.emailInput    = page.getByPlaceholder('Email Address');
    this.passwordInput = page.getByPlaceholder('Password');
    // Submit button is type="submit", text "Sign In" — no trailing space
    this.signInBtn     = page.getByRole('button', { name: 'Sign In', exact: true });
    // Eye icon is the only type="button" on the login form
    this.eyeIcon       = page.locator('button[type="button"]').first();
    this.forgotLink    = page.getByRole('link', { name: 'Forgot Password' });
    this.signUpLink    = page.getByRole('link', { name: 'Sign Up' });
    // ── Validation messages ────────────────────────────────────────────
    this.emailRequiredMsg    = page.getByText('Email address is required.');
  ...
  async open() {
    await this.goto(ROUTES.LOGIN);
    // Wait for the form to mount — React SPA needs time after navigation
    await this.emailInput.waitFor({ state: 'visible', timeout: 30_000 });
  }
